...
export class SelectDropDownComponent implements OnInit, OnChanges, AfterViewInit {
   ...
   * @param index:  index of the item
   */
  public selectItem(item: any,test: boolean, i?: number) {
    this.classActive = item;
    this.toggleDropdown = false;
    //this.selectedDisplayText = item.display;
    // if (!this.multiple) {
    //   if (this.selectedItems.length > 0) {
   ...
   // this.selectedItems.sort(this.config.customComparator);
    ...
    this.valueChanged();
    this.resetArrowKeyActiveElement();
    this.setSelectedDisplayText(item.display);
    //this.test = false;
  //  }
  }
  ...
  private initDropdownValuesAndOptions() {
    ...
    this.setSelectedDisplayText("undefined");
  }

  /**
   * set the text to be displayed
   */
  private setSelectedDisplayText(item) {
     this.selectedDisplayText = (item == "undefined") ? this.config.placeholder : item;
